...
df = (
    spark.sql("SELECT * FROM lh_edp_prod.stream.orderupdated_orderstream ")
    .withColumn("order_bk",col('orderNumber'))
    # .withColumn("payment_bk",concat_ws('_',col('orderNumber'),col('title'),col('psp'),col('method'),col('pspReference')))
)
# display(df)

# df.write.format("delta").option("overwriteSchema", "true").mode('overwrite').saveAsTable("stream.orderupdated_orderstream")
...
checkpoint_base = "Files/checkpoints/sg_orderupdated_"
checkpoint_name = "multi_silver"


silver_tables = {
    "order":     {"table": "sg_orderupdated_order",     "keys": ["order_hashkey"]},
    ...
    "row":       {"table": "sg_orderupdated_row",       "keys": ["row_hashkey"]},
...
def ensure_lock_table():
    if not spark.catalog.tableExists(lock_table):
        spark.sql(f"""CREATE TABLE IF NOT EXISTS {lock_table} (
            ...
            USING DELTA""")
        print(f"Created lock table {lock_table}")
...
def acquire_lock(lock_name: str, ttl_minutes: int = 30) -> str | None:
    ...
    if result and result[0]["owner"] == lock_owner:
        print(f"🔒 Lock acquired: {lock_name} by owner {lock_owner}")
        return lock_owner
    else:
        print(f"⚠️ Failed to acquire lock: {lock_name}, held by {result[0]['owner'] if result else 'unknown'}")
        return None
...
def release_lock(lock_name: str, lock_owner: str) -> bool:
    ...
    if result.count() == 0:
        print(f"⚠️ Release failed: {lock_name} not owned by {lock_owner} or lock missing")
        return False
    else:
        print(f"🔓 Lock released: {lock_name} by owner {lock_owner}")
        return True
...
def cleanup_expired_locks():
    ...
    now = datetime.utcnow()
    print(f"🧹 Cleaning up locks older than {now} (UTC)...")
    spark.sql(f"""
        DELETE FROM {lock_table}
        WHERE expiry_ts <= current_timestamp()
    """)
    remaining = spark.sql(f"SELECT * FROM {lock_table}").count()
    print(f"✅ Cleanup complete. Remaining active locks: {remaining}")
...
def delete_all_locks():
    ...
    spark.sql(f"""
        DELETE FROM {lock_table}
    """)
    print(f"✅ Remove all locks complete.")
...
def merge_to_silver(df, table_name, keys):
    if df.rdd.isEmpty():
        print(f"⚠️ No rows for {table_name} in this batch")
        return
    ...
    if not spark.catalog.tableExists(table_name):
        ...
        print(f"{datetime.now()} ℹ️ Creating Silver table {table_name} DONE")
        return
...
def upsert_all_silver(batch_df, batch_id):
    print(f"{datetime.now()}⚡ upsert_all_silver start")
...
print(f"{datetime.now()} ℹ️ Execution starts")
...
# spark.sql("OPTIMIZE stream.orderupdated_raw")
# 14 min
...
# spark.sql("VACUUM stream.orderupdated_raw")
...
#spark.sql("DROP TABLE dbo.sg_orderupdated_row")
#spark.sql("CREATE TABLE dbo.sg_orderupdated_row AS SELECT * FROM dbo.orderupdated_row")
spark.sql("CREATE OR REPLACE TABLE dbo.sg_orderupdated_order AS SELECT * FROM dbo.orderupdated_order")
...
#spark.sql("refresh TABLE dbo.sg_orderupdated_row")
...
# MAGIC %%sql
# MAGIC SELECT
# MAGIC   orderNumber , orderDate , EventEnqueuedUtcTime, buckets, len(buckets)
# MAGIC FROM stream.orderupdated_raw
# MAGIC where orderNumber='100-25296464'
